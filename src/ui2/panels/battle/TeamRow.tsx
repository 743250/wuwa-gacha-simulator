...
export function TeamRow({ battle }: TeamRowProps) {
  const aliveTeam = battle.team.filter((t: any) => t.alive);

  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${aliveTeam.length},1fr)`, gap: 6 }}>
      {battle.team.map((t: any, i: number) => {
