...
//   · 按钮 onClick 走 window.__podcast.xxx() —— 老 handler 还会调 renderPodcast(),
//     但 renderPodcast 已变成 no-op,最终 bumpStateVersion() 驱动本组件刷新
      ...
      <div class={freeCls}
        ...
        onClick={() => {
          if (reached && !freeClaimed) {
            (window as any).__podcast.claimFree(lv);
            (window as any).__rerenderAll?.();
          }
        }}
      ...
      <div class={paidCls}
        ...
        onClick={() => {
          if (reached && !paidClaimed && !lockedPaid) {
            (window as any).__podcast.claimPaid(lv);
            (window as any).__rerenderAll?.();
          }
        }}
...
function TaskItem({ id, name, exp }: any) {
  ...
  return (
    <div class={cls} style={{
      ...
      <div class="pct-info">{Math.min(st.progress, st.target)}/{st.target} · +{exp} EXP {st.done ? '✓' : ''}</div>
    </div>
  );
}
    ...
    <div>
      ...
      <div class="pc-head">
        ...
        <div class="pc-purchase">
          {p.paid
            ...
            : <button class="mbtn gold" onClick={() => (window as any).buyShop('bp_basic')}>¥68 解锁内幕频道</button>}
          {p.premium
            ? <span class="tag-on">寰宇频道</span>
            : <button class="mbtn" onClick={() => (window as any).buyShop('bp_premium')}>¥128 寰宇频道</button>}
        </div>
      </div>
      ...
      <div class="pc-progress">
        ...
        <div class="pc-actions">
          <button class="mbtn" onClick={() => { (window as any).__podcast.buyLevel(1); (window as any).__rerenderAll?.(); }}>
            买 1 级 (¥声 {PODCAST_BUY_LEVEL_COST})
          </button>
          <button class="mbtn" onClick={() => { (window as any).__podcast.buyLevel(5); (window as any).__rerenderAll?.(); }}>
            买 5 级 (¥声 {PODCAST_BUY_LEVEL_COST * 5})
          </button>
          <button class="mbtn gold" onClick={() => { (window as any).__podcast.claimAll(); (window as any).__rerenderAll?.(); }}>
            一键领取已达成
          </button>
        </div>
      </div>
