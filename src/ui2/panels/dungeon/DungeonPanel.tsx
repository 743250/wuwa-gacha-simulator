...
import {
  DUNGEONS, WEEKLY_BOSS, WEEKLY_BOSS_LIMIT, SOL3_LEVELS,
...
(WEEKLY_BOSS as any[]).forEach((b: any) => {
  if (!(DUNGEONS as any[]).find((x: any) => x.id === b.id)) (DUNGEONS as any[]).push(b);
});
    ...
    <article class={cardClass}>
      <div class="dng-card-top">
        ...
        <button class="mbtn gold"
          disabled={!canAfford}
          onClick={() => (window as any).__startDungeon?.(d.id)}>
