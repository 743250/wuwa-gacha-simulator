...
function WeaponCard({ name, weapon }: any) {
  ...
  return (
    <div class={`role r${r}`}
      onClick={() => (window as any).__openWeaponModal?.(name)}
      style={{
    ...
    <div class="echo-card"
      ...
      <div style={{ display: 'flex', gap: 3, marginTop: 5, flexWrap: 'wrap', justifyContent: 'center' }}
        ...
        <button class="mbtn" style={{ fontSize: 9, padding: '2px 6px' }}
          onClick={() => (window as any).__bagEchoLevelUp?.(echo.id)}>升级</button>
        <button class="mbtn gold" style={{ fontSize: 9, padding: '2px 6px' }}
          onClick={() => (window as any).__bagEchoLevelUpMax?.(echo.id)}>升满</button>
        <button class="mbtn" style={{ fontSize: 9, padding: '2px 6px' }}
          onClick={() => (window as any).__bagEchoToggleLock?.(echo.id)}>{echo.lock ? '解锁' : '锁定'}</button>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      {echos.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(110px,1fr))', gap: 6 }}>
          {echos.map((e: any) => <EchoCard key={e.id} echo={e} />)}
        </div>
      )}
    </div>
  );
