...
export function PullPanel() {
  ...
  const lbl1 = `× 1 · ${tideHave > 0 ? '消耗 1 ' + tn : '消耗 160 星声'}`;
  const lbl10Parts: string[] = [];
  if (tideUsed10 > 0) lbl10Parts.push(tideUsed10 + ' ' + tn);
  const astShow = Math.min(astUsed10, astHave);
  if (astShow > 0) lbl10Parts.push(astShow.toLocaleString() + ' 星声');
  if (lunNeed10 > 0) lbl10Parts.push(lunNeed10.toLocaleString() + ' 月相');
  const lbl10 = `× 10 · 消耗 ${lbl10Parts.length ? lbl10Parts.join(' + ') : '0'}`;
