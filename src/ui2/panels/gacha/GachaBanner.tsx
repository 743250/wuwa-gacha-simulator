    ...
    <Fragment>
      ...
      <div class={'banner-art ' + (kind === 'weapon' ? 'theme-l' : 'theme-r')} id="bnArt">
        <div class="ba-main">
          ...
          {targetOptions(b) ? <div dangerouslySetInnerHTML={{ __html: targetOptions(b) }} /> : null}
...
function BannerPreviewButtons({ b, kind }: any) {
  ...
  if (b.char) {
    buttons.push(
      <button class="mbtn gold" onClick={(e: any) => { e.stopPropagation(); (window as any).openRolePreview(b.char); }}>
        查看 {b.char}
      </button>
    );
  }
  if (kind === 'weapon' && b.weapon) {
    buttons.push(
      <button class="mbtn" onClick={(e: any) => { e.stopPropagation(); (window as any).__openWeaponInfo(b.weapon); }}>
        查看武器
      </button>
    );
  }
  if ((b.pool === 'standardChar' || b.pool === 'beginner') && !b.char) {
    buttons.push(
      <button class="mbtn" onClick={(e: any) => { e.stopPropagation(); (window as any).__openStandardRolePreview(); }}>
        查看常驻五星
      </button>
    );
  }
