...
import { doCommission, claimTour } from './actions';
  ...
  return (
    <div>
      <div style={{ textAlign: 'center', marginBottom: 14 }}>
        ...
        <div style={{ margin: '6px auto 0', maxWidth: 220, height: 4, background: 'rgba(255,255,255,.08)', borderRadius: 2, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${cs.length ? (doneCount / cs.length * 100) : 0}%`, background: 'linear-gradient(90deg,var(--accent),var(--gold))', transition: 'width .3s' }} />
        </div>
        {doneCount < cs.length ? (
          <button class="mbtn gold" style={{ marginTop: 8, fontSize: 11, padding: '5px 14px' }}
            onClick={() => { cs.forEach((c: any, i: number) => { if (!c.done) doCommission(i); }); }}>
            一键完成全部
          </button>
        ) : null}
      </div>
