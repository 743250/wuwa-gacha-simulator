...
import { levelUpRole, levelUpRoleMax } from '../../../equip/actions.js';
...
function doLevelUpRole(roleName: string) {
  if (levelUpRole(roleName)) {
    msg(`${roleName} 升级成功`, false);
    bumpStateVersion();
  }
}

function doLevelUpRoleMax(roleName: string) {
  const n = levelUpRoleMax(roleName);
  if (n > 0) {
    msg(`${roleName} 一键升至 Lv 90（升 ${n} 级）`, false);
    bumpStateVersion();
  } else {
    msg('材料不足或已满级');
  }
      ...
      <div style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '11px 13px', background: 'rgba(255,255,255,.02)', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          <button class={`mbtn ${expTotal >= expNext && level < 90 ? 'gold' : ''}`}
            onClick={() => doLevelUpRole(roleName)}
            disabled={level >= 90 || expTotal < expNext}
            style={{ flex: 1 }}>
            {level >= 90 ? '已满级' : expTotal < expNext ? `材料不足 · 差 ${(expNext - expTotal).toLocaleString()} exp` : `升 1 级 (${expNext.toLocaleString()} exp)`}
          </button>
          <button class="mbtn"
            onClick={() => doLevelUpRoleMax(roleName)}
            disabled={level >= 90 || expTotal < expNext}
            style={{ flex: 1 }}>
            一键升满
          </button>
