...
import { openTeamPicker, toggleTeamMember, swapTeamMember } from './actions';
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
        <button class="mbtn" style={{ fontSize: 11, padding: '4px 8px' }}
          disabled={slotIdx === 0}
          onClick={() => swapTeamMember(slotIdx, slotIdx - 1)}>◀</button>
        <button class="mbtn" style={{ flex: 1, fontSize: 11, padding: '5px' }}
          onClick={() => openTeamPicker(slotIdx)}>
          {name ? '更换' : '选择'}
        </button>
        <button class="mbtn" style={{ fontSize: 11, padding: '4px 8px' }}
          disabled={slotIdx >= TEAM_SIZE - 1}
          onClick={() => swapTeamMember(slotIdx, slotIdx + 1)}>▶</button>
      </div>
    </div>
  );
