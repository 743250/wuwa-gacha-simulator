...
export const PODCAST_TASKS = {
  ...
  // 42 天 × 330 ≈ 14 级（仅每日）
  // 注：d_upgrade 现覆盖 角色/武器/声骸 三类升级（见 equip/*.js progressTask 调用）
  daily: [
    ...
    { id: 'd_upgrade',   name: '升级角色/武器/声骸 1 次', exp: 40, target: 1 }
  ],
  // ============ 每周（合计 ~3000 EXP / 周 = 3 级）============
  // 6 周 × 3000 = 18 级
  weekly: [
    { id: 'w_commission',  name: '累计完成 21 个委托', exp: 400, target: 21 },
    { id: 'w_dungeon',     name: '通关 3 次副本',       exp: 400, target: 3  },
    { id: 'w_weeklyboss',  name: '击败 1 次周 BOSS',    exp: 700, target: 1  },
    { id: 'w_abyss',       name: '深塔通关 5 层',       exp: 700, target: 5  },
    { id: 'w_levelup',     name: '角色累计升级 +10',    exp: 400, target: 10 },
    { id: 'w_echo',        name: '声骸累计升级 15 次',  exp: 400, target: 15 }
  ],
  // ============ 本期（一个版本周期一次性 ≈ 35 级）============
  // 大件本期任务给大头，鼓励玩家把游戏内容做完
  // 注：删除 p_char90（角色有限，养满一个后续版本无法重复），改为可重复的声骸凑套任务
  period: [
    ...
    { id: 'p_echo',        name: '声骸累计升级 60 次', exp: 3000, target: 60  },
