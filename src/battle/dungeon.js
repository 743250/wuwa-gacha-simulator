...
// ===== 世界等级（3 档 · 三档等级区间机制）=====
// Ⅰ=30-60 / Ⅱ=60-90 / Ⅲ=90-120，每副本独立追踪等级
// 初始=本档下限 · 胜利 +10 · 失败 -20 · 切档重置到新档下限
export const SOL3_LEVELS = {
  1: { name: '世界等级 1', levelMin: 30, levelMax: 60, dropMult: 1.0 },
  2: { name: '世界等级 2', levelMin: 60, levelMax: 90, dropMult: 2.0 },
  3: { name: '世界等级 3', levelMin: 90, levelMax: 120, dropMult: 3.0 }
};
...
// 副本等级追踪
export function getDungeonEnemyLevel(d) {
  if (!d) return SOL3_LEVELS[getSol3Level()].levelMin;
  if (!S.dungeonLevels) S.dungeonLevels = {};
  const tier = SOL3_LEVELS[getSol3Level()];
  const cur = S.dungeonLevels[d.id];
  if (cur == null) return tier.levelMin;
  // 钳到当前档位区间（防切档后越界）
  return Math.max(tier.levelMin, Math.min(tier.levelMax, cur));
}

export function onBattleResult(d, result) {
  if (!d || !result) return;
  if (!S.dungeonLevels) S.dungeonLevels = {};
  const tier = SOL3_LEVELS[getSol3Level()];
  const cur = S.dungeonLevels[d.id] ?? tier.levelMin;
  let next = cur;
  if (result === 'win') next = cur + 10;
  else if (result === 'lose') next = cur - 20;
  S.dungeonLevels[d.id] = Math.max(tier.levelMin, Math.min(tier.levelMax, next));
  return S.dungeonLevels[d.id];
}

// 获取世界 BOSS 的 spawnEnemy 参数（沿用旧接口，内部走三档等级）
