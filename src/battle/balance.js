...
// 深塔”水温”：官方中一血量膨胀曲线（相对倍率）+ 绝对锚（仅乘 HP）
//
// 官方实测数据点（中塔 1 层 · 社区/资料）：
//   1.0 开服：中一 ~160 万 → 相对 1.00
//   1.4 末期：中一 ~214 万 → 相对 1.34
//   2.7 末期：中一 ~291 万 → 相对 ~1.82（表内 2.14 为插值路径）
//   3.4 当前：中一 ~550 万 → 相对 3.44
//
// 相对字段 hp/atk/def：版本间相对膨胀（曲线保留）。
// 危险区实装只把 temp.hp 乘进敌人生命；攻防用 core，不跟版本水温涨。
//
// 绝对锚 ABYSS_HP_ABS_ANCHOR：仅乘危险区 HP。
// 相对曲线仍用上表 1.00→3.44；锚负责把 encore×塔压 拉到「后期顶关约 500–600 万」。
//
// 标定（3.4 · 中塔 4 · 叹息古龙）：
//   无锚：953575 × 1.15 × 0.68 × 3.44 ≈ 256.5 万
//   目标约 550 万 → 锚 ≈ 550/256.5 ≈ 2.15
// 同锚下约：
//   3.4 中 1 燎照 ≈ 420 万；中 4 古龙/赫卡忒 ≈ 550–580 万（单只顶格）
// 不再按「中一必须 550 万」硬锚（会把中 4 顶到 700 万+，偏高）。
//
// 曲线特征：1.x 平缓，2.x 温和，3.x 加速
// 旧角色打不动新深塔 = 逼你抽新角色 = 逼氪闭环
export const ABYSS_HP_ABS_ANCHOR = 2.15;
